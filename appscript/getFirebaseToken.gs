...
function getFirebaseIdToken() {
  try {
    ...
    const tokenData = fetchTokenForSheet();
    
    Logger.log('✅ Đã lấy token thành công!');
    Logger.log('Token expires in: ' + tokenData.expiresIn + ' seconds');
    
    // Ghi token vào sheet
    writeTokenDataToSheet(tokenData);
    
    // Hiển thị thông báo
    SpreadsheetApp.getActiveSpreadsheet().toast(
      'Token đã được lưu vào sheet "' + TOKEN_FIREBASE_CONFIG.SHEET_NAME + '"',
      '✅ Thành công',
      5
    );
 ...
 */
function fetchTokenForSheet() {
  const url = TOKEN_FIREBASE_CONFIG.API_URL + '?key=' + TOKEN_FIREBASE_CONFIG.API_KEY;
  
  const payload = {
    returnSecureToken: true,
    email: TOKEN_FIREBASE_CONFIG.EMAIL,
    password: TOKEN_FIREBASE_CONFIG.PASSWORD,
 ...
 */
function writeTokenDataToSheet(tokenData) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(TOKEN_FIREBASE_CONFIG.SHEET_NAME);
  
  // Tạo sheet mới nếu chưa có
  if (!sheet) {
    sheet = ss.insertSheet(TOKEN_FIREBASE_CONFIG.SHEET_NAME);
  }
  ...
  Logger.log('💾 Đã ghi token vào sheet "' + TOKEN_FIREBASE_CONFIG.SHEET_NAME + '"');
}
...
function showTokenHelp() {
  const html = HtmlService.createHtmlOutput(`
    ...
    <h3>1. Cấu hình:</h3>
    <p>Cập nhật <code>EMAIL</code> và <code>PASSWORD</code> trong biến <code>TOKEN_FIREBASE_CONFIG</code></p>
