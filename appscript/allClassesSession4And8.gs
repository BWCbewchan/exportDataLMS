/**
 * ALL CLASSES SESSION 4 & 8 FETCHER
 * 
 * LẤY TẤT CẢ LỚP HỌC CÓ BUỔI 4 VÀ BUỔI 8 TRONG THÁNG CHỈ ĐỊNH
 * Script lấy toàn bộ lớp học (không phân biệt khóa) có buổi 4 hoặc buổi 8 trong tháng cụ thể
 * 
 * THAM SỐ:
 ...
 * 
 * OUTPUT:
 * - Sheet 1: "All Classes" - Toàn bộ lớp đang RUNNING
 * - Sheet 2: "All Classes Session 4 & 8" - Lọc lớp có buổi 4/8 trong tháng
 */
...
const ALL_CLASSES_CONFIG = {
  API_URL: 'https://lms-api.mindx.vn/',
  AUTH_TOKEN: '', // Sẽ được tự động cập nhật
  SHEET_ALL_CLASSES: 'All Classes',  // Sheet chứa TẤT CẢ lớp
  SHEET_SESSION_4_8: 'All Classes Session 4 & 8', // Sheet chỉ lọc buổi 4&8
...
/**
 * Hàm chính: 
 * 1. Fetch TOÀN BỘ dữ liệu lớp học về trước
 * 2. Ghi tất cả lớp vào sheet "All Classes"
 * 3. Lọc và ghi lớp có buổi 4 & 8 vào sheet "All Classes Session 4 & 8"
 * 
 ...
 */
function fetchAllClassesSession4And8(targetMonth, targetYear) {
  try {
    ...
    clearAllClassesSheets();
    
    // BƯỚC 2: Chuẩn bị Token
    Logger.log('🔐 Lấy Firebase token...');
    ALL_CLASSES_CONFIG.AUTH_TOKEN = getAllClassesFirebaseToken();
    
    // BƯỚC 3: FETCH LẠI TOÀN BỘ DANH SÁCH LỚP HỌC
    Logger.log('🚀 Đang lấy lại TOÀN BỘ danh sách lớp học từ API...');
    ss.toast('🚀 Đang lấy lại danh sách ALL Classes...', 'Fetching API', 3);
    
    const allClasses = fetchAllClassesData();
    Logger.log(`✅ Đã fetch xong: ${allClasses.length} lớp học.`);
    ss.toast(`✅ Đã lấy được ${allClasses.length} lớp học!`, 'Fetch Complete', 2);
    
    // BƯỚC 4: GHI TẤT CẢ LỚP VÀO SHEET 1
    Logger.log(`📝 Đang ghi ${allClasses.length} lớp vào sheet "${ALL_CLASSES_CONFIG.SHEET_ALL_CLASSES}"...`);
    ss.toast(`📝 Đang ghi ${allClasses.length} lớp vào "All Classes"...`, 'Writing Data', 2);
    writeAllClassesToAllSheet(allClasses);
    Logger.log(`✅ Đã ghi xong sheet "All Classes"`);
    ...
    const filteredClasses = filterAllClassesSession4And8(allClasses, month, year);
    Logger.log(`📝 Đang ghi ${filteredClasses.length} lớp có buổi 4&8 vào sheet "${ALL_CLASSES_CONFIG.SHEET_SESSION_4_8}"...`);
    writeAllClassesSession4And8ToSheet(filteredClasses, month, year);
    Logger.log(`✅ Đã ghi xong sheet "All Classes Session 4 & 8"`);
    
    // BƯỚC 6: THÔNG BÁO HOÀN THÀNH
    Logger.log(`\n🎉 HOÀN THÀNH!`);
    Logger.log(`   - Tổng số lớp học: ${allClasses.length} (đã refresh từ API)`);
    Logger.log(`   - Lớp có buổi 4/8 tháng ${month}/${year}: ${filteredClasses.length}`);
    
    ss.toast(
      `✅ Tháng ${month}/${year}: ${allClasses.length} lớp (mới), ${filteredClasses.length} lớp có buổi 4/8!`, 
      'Hoàn thành', 
      5
    );
 ...
 */
function clearAllClassesSheets() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  
  // Clear sheet All Classes
  let sheet = ss.getSheetByName(ALL_CLASSES_CONFIG.SHEET_ALL_CLASSES);
  if (sheet) {
    sheet.clear();
    Logger.log(`   ✓ Đã xóa "${ALL_CLASSES_CONFIG.SHEET_ALL_CLASSES}"`);
  }
  
  // Clear sheet All Classes Session 4 & 8
  sheet = ss.getSheetByName(ALL_CLASSES_CONFIG.SHEET_SESSION_4_8);
  if (sheet) {
    sheet.clear();
    Logger.log(`   ✓ Đã xóa "${ALL_CLASSES_CONFIG.SHEET_SESSION_4_8}"`);
  }
}
...
function fetchAllClassesData() {
  ...
  do {
    ...
    let response;
    try {
      response = fetchAllClassesWithRetry(page);
    } catch (e) {
    ...
    if (classData.length < ALL_CLASSES_CONFIG.RECORDS_PER_PAGE) {
       hasMore = false;
    }
    
    page++;
    Utilities.sleep(ALL_CLASSES_CONFIG.SLEEP_TIME); // Nghỉ xíu tránh 502
 ...
 */
function fetchAllClassesWithRetry(page) {
  const makeRequest = () => {
    const variables = {
        search: "",  // Không filter search - lấy tất cả
        centres: [],
        courses: [],
        courseLines: [],  // Không filter courseLines - lấy tất cả
        startDate: [null, null],
        endDate: [null, null],
        statusIn: ["RUNNING"],
        pageIndex: page,
        itemsPerPage: ALL_CLASSES_CONFIG.RECORDS_PER_PAGE,
    ...
    const options = {
        ...
        headers: {
            'accept': '*/*',
            'authorization': ALL_CLASSES_CONFIG.AUTH_TOKEN,
    ...
    return UrlFetchApp.fetch(ALL_CLASSES_CONFIG.API_URL, options);
  };

  let attempts = 0;
  let lastError;
  
  while (attempts < ALL_CLASSES_CONFIG.MAX_RETRIES) {
    try {
      if (attempts > 0) {
      ...
      if (code === 401) {
        ...
        try {
           ALL_CLASSES_CONFIG.AUTH_TOKEN = getFirebaseIdToken(); 
...
/**
 * Ghi TẤT CẢ lớp vào sheet 1
 */
function writeAllClassesToAllSheet(allClasses) {
  const sheet = prepareAllClassesSheet(ALL_CLASSES_CONFIG.SHEET_ALL_CLASSES);
  
  // Header
  const headers = ['Tên lớp', 'Khóa học', 'Cơ sở', 'Status', 'Số HS', 'LEC', 'Số buổi', 'Giờ học'];
  ...
  const rows = allClasses.map(cls => {
    ...
    return [className, courseName, centreName, status, totalStudents, allTeachers, totalSlots, uniqueTimeSlots];
  });
  
  if (rows.length > 0) {
    sheet.getRange(2, 1, rows.length, rows[0].length).setValues(rows);
    formatAllClassesSheet(sheet, rows.length + 1, headers.length, 4); // Column 4 = Status
  }
}
 ...
 */
function filterAllClassesSession4And8(allClasses, targetMonth, targetYear) {
  const filtered = [];
  ...
  allClasses.forEach(cls => {
    ...
    cls.slots.forEach((slot, index) => {
      ...
      if (sessionNumber === 4) {
        session4 = slot;
        if (isInTargetMonthAllClasses(slot.date, targetMonth, targetYear)) session4InMonth = true;
      }
      if (sessionNumber === 8) {
        session8 = slot;
        if (isInTargetMonthAllClasses(slot.date, targetMonth, targetYear)) session8InMonth = true;
      }
    });
 ...
 */
function writeAllClassesSession4And8ToSheet(filteredClasses, targetMonth, targetYear) {
  const sheet = prepareAllClassesSheet(ALL_CLASSES_CONFIG.SHEET_SESSION_4_8);
  
  // Header với thông tin tháng/năm
  const title = `Tất cả lớp có buổi 4/8 - Tháng ${targetMonth}/${targetYear}`;
  ...
  const headers = ['Tên lớp', 'Khóa học', 'Cơ sở', 'Buổi 4', 'Buổi 8', 'LEC', 'Số HS', 'Status', 'Giờ học'];
  ...
  const rows = filteredClasses.map(item => {
    ...
    const session4Date = item.session4InMonth ? formatAllClassesSessionDate(item.session4.date) : '';
    const session8Date = item.session8InMonth ? formatAllClassesSessionDate(item.session8.date) : '';
    ...
    return [className, courseName, centreName, session4Date, session8Date, allTeachers, totalStudents, status, uniqueTimeSlots];
  });
  
  sheet.getRange(3, 1, rows.length, rows[0].length).setValues(rows); // Row 3 vì row 1 là title
  formatAllClassesSheet(sheet, rows.length + 2, headers.length, 8); // +2 vì có title row, column 8 = Status (vẫn giữ index vì Status vẫn ở vị trí cũ)
}

/**
 * Chuẩn bị sheet (clear và tạo mới nếu cần)
 */
function prepareAllClassesSheet(sheetName) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
 ...
 * Format sheet chung
 */
function formatAllClassesSheet(sheet, lastRow, lastCol, statusCol) {
  ...
  const hasTitle = (sheet.getName() === ALL_CLASSES_CONFIG.SHEET_SESSION_4_8);
 ...
 */
function isInTargetMonthAllClasses(dateStr, targetMonth, targetYear) {
...
function formatAllClassesSessionDate(dateStr) {
  if (!dateStr) return '';
  try {
    const date = new Date(dateStr);
...
function getAllClassesFirebaseToken() {
  const tokenFromSheet = readAllClassesTokenFromSheet();
  if (tokenFromSheet) return tokenFromSheet;
  return getFirebaseIdToken();
}

function readAllClassesTokenFromSheet() {
  try {
...
function fetchAllClassesCurrentMonth() {
  const now = new Date();
  fetchAllClassesSession4And8(now.getMonth() + 1, now.getFullYear());
}

// Lấy dữ liệu tháng TRƯỚC
function fetchAllClassesLastMonth() {
  const now = new Date();
  ...
  fetchAllClassesSession4And8(month, year);
}

// Lấy dữ liệu tháng SAU
function fetchAllClassesNextMonth() {
  const now = new Date();
...
function fetchAllClassesJanuary2026() {
  fetchAllClassesSession4And8(1, 2026);
}

// Lấy dữ liệu THÁNG 2/2026
function fetchAllClassesFebruary2026() {
  fetchAllClassesSession4And8(2, 2026);
}

// Lấy dữ liệu THÁNG 3/2026
function fetchAllClassesMarch2026() {
  fetchAllClassesSession4And8(3, 2026);
}
 ...
 * Tạo menu custom
 */
function onOpenAllClassesSession4And8Menu() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('📚 All Classes S4&8')
    .addItem('🏙️ ALL TP - Tất cả cơ sở', 'fetchAllClassesCurrentMonth')
    .addSeparator()
    .addItem('🚀 Tháng hiện tại', 'fetchAllClassesCurrentMonth')
    .addItem('⏮️ Tháng trước', 'fetchAllClassesLastMonth')
    .addItem('⏭️ Tháng sau', 'fetchAllClassesNextMonth')
    .addSeparator()
    .addItem('1️⃣ Tháng 1/2026', 'fetchAllClassesJanuary2026')
    .addItem('2️⃣ Tháng 2/2026', 'fetchAllClassesFebruary2026')
    .addItem('3️⃣ Tháng 3/2026', 'fetchAllClassesMarch2026')
    .addSeparator()
    .addItem('📆 Chọn tháng/năm tùy chỉnh...', 'showAllClassesCustomMonthDialog')
    .addToUi();
}
 ...
 */
function showAllClassesCustomMonthDialog() {
  const ui = SpreadsheetApp.getUi();
  const now = new Date();
